...
object RpcClient:
  def apply[F[_]](nodeId: NodeId, client: Client[F])(using F: Concurrent[F]): ExternalNode[F, KvCommand] =
    new:
      val id: NodeId = nodeId

      def appendEntries(req: AppendEntries[KvCommand]): F[AppendResponse] =
        client.expect(Request(Method.PUT, nodeId.toUri / "raft" / "append_entries").withEntity(req))

      def requestVote(req: RequestVote): F[VoteResponse] =
        client.expect(Request(Method.PUT, nodeId.toUri / "raft" / "request_vote").withEntity(req))
